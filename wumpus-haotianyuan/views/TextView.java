...
/*Author:Haotian Yuan
 * 
 * This class is to draw the textview of the game
 */
...
public class TextView extends BorderPane implements Observer{
	...
	public void initializePane() {
		...
		gc.fillText(text, 0, 260);//add the label text
	...
	public void updatemap() {
		...
		gc.clearRect(0, 0, 240, 260);//clear the screen firstly
		gc.fillText(text, 0, 260);//add the label
		...
		public void handle(KeyEvent event) {
			...
			if(a.compareTo("RIGHT")==0) {//if we press right, the hunter go right
				if(available[x][y]=='1') {
					...
					move[x][y]='1';
					if(theGame.boold(x, y)==true) {//make some warning for users
